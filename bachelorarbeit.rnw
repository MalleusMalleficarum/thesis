...
Two verticies $u, v$ are adjacent if $\exists e \in E | u, v \in e$ and a vertex $u$ is incident to a hyperedge $e$ if $ u \in e$. The size $|e|$ of a hyperedge $e$ is the number of vertices contained in $e$. A k-way partition of a hypergraph $H$ is a partition of $V$ into k disjoint blocks $V_1, .. V_k$. $part: V \rightarrow [0, k-1]$ is a function referencing the corresponding block of a k-way partition to a vertex $u$. 
...
A valid solution is a balanced k-way partition. An invalid solution is a partition where either the balance criterion is not met.
...
\begin{center}
<<replacestrategies, fig=TRUE, echo=FALSE, height=4, width=6>>=
...
&\multicolumn{2}{c||}{KaHyPar-E+C1+C2} & \multicolumn{2}{c}{KaHyPar+M1+M2} \\
